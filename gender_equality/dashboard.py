...
st.markdown("**Excel analysis:** [waiting Shauryaman](waiting Shauryaman)")
st.markdown("---")
...
# Computation for a first interpretation
max_gap_row = pay_gap_data.loc[pay_gap_data['Gap_Percentage'].idxmax()]
...
max_bonus_gap_dept = variable_pivot['Gap'].idxmax()
...
fig7 = px.box(df, x='Sexe', y='Salaire base mensuel',
...
st.write(f"**Interpretation:** The median salary for men ({male_median:.0f}€) is {distribution_gap:.1f}% higher than for women ({female_median:.0f}€). Women are concentrated in lower salary brackets while men dominate the upper ranges.")
...
if len(part_time_data) > 0:
    ...
    fig8 = px.bar(hourly_data, x='Sexe', y='Hourly_Rate',
                 title="Hourly Rate for Part-Time Workers")
    st.plotly_chart(fig8, use_container_width=True)
    
    if len(hourly_data) == 2:  # Les deux genres représentés
        male_hourly = hourly_data[hourly_data['Sexe']=='H']['Hourly_Rate'].values[0]
        female_hourly = hourly_data[hourly_data['Sexe']=='F']['Hourly_Rate'].values[0]
        hourly_gap = ((male_hourly - female_hourly) / male_hourly) * 100
        st.write(f"**Interpretation:** Part-time female workers earn {hourly_gap:.1f}% less per hour than their male counterparts, indicating a 'part-time penalty' that compounds gender inequality.")
else:
    st.write("No part-time workers in the dataset")

st.markdown("---")

# 9. Satisfaction vs Compensation (sans trendline pour éviter l'erreur)
...
correlation_m = df[df['Sexe']=='H'][['Salaire base mensuel', 'Niveau de satisfaction']].corr().iloc[0,1]
correlation_f = df[df['Sexe']=='F'][['Salaire base mensuel', 'Niveau de satisfaction']].corr().iloc[0,1]
st.write(f"**Interpretation:** Salary-satisfaction correlation is {correlation_m:.2f} for men vs {correlation_f:.2f} for women. This difference suggests gender-based variations in how compensation relates to job satisfaction.")
...
# Calculer l'évolution de l'écart
young_gap = ((age_salary_data[(age_salary_data['Age']=='20-35') & (age_salary_data['Sexe']=='H')]['Salaire base mensuel'].values[0] - 
              age_salary_data[(age_salary_data['Age']=='20-35') & (age_salary_data['Sexe']=='F')]['Salaire base mensuel'].values[0]) / 
             age_salary_data[(age_salary_data['Age']=='20-35') & (age_salary_data['Sexe']=='H')]['Salaire base mensuel'].values[0]) * 100

old_gap = ((age_salary_data[(age_salary_data['Age']=='46-55') & (age_salary_data['Sexe']=='H')]['Salaire base mensuel'].values[0] - 
            age_salary_data[(age_salary_data['Age']=='46-55') & (age_salary_data['Sexe']=='F')]['Salaire base mensuel'].values[0]) / 
           age_salary_data[(age_salary_data['Age']=='46-55') & (age_salary_data['Sexe']=='H')]['Salaire base mensuel'].values[0]) * 100

gap_increase = old_gap - young_gap
st.write(f"**Interpretation:** The gender pay gap increases from {young_gap:.1f}% for 20-35 year olds to {old_gap:.1f}% for 46-55 year olds, a {gap_increase:.1f}% point widening. This demonstrates how early-career disparities compound over time.")
